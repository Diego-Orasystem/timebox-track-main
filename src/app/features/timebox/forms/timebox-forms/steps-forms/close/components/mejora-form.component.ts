  ...
  >
    ...
    >
      ...
      <div class="content-modal w-full px-4 py-8 flex flex-col gap-4">
        <h1 class=" text-[14px] font-semibold">Añadir Mejora</h1>

        <div class="eje flex flex-col gap-2">
          <label class=" text-[12px]" for="aplicativo">Tipo mejora</label>
...
export class MejoraFormComponent {
  @Input() show = false;
  ...
  opciones = ['Mejoras críticas', 'Mejoras funcionales', 'Mejoras UX'];
  ...
  addMejora() {
    if (!this.nombreMejora.trim() || !this.tipeSeleccionado.trim()) return;
    this.info.emit({
      tipo: this.tipeSeleccionado,
      nombre: this.nombreMejora,
    });
